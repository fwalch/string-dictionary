...
template <class T> const std::string &refX(const T &t);

template <class T>
const std::string &refY(const std::string &s) {
    return s;
}

template <class T>
const std::string &refX(const std::pair<std::string, T> &p) {
    return p.first;
}
...
struct htnode {
    htnode(char ch = '\0') : ch(ch), parent(NULL) {
        memset(children, 0, sizeof(child_ptr) * HT_ALPHABET_SIZE);
    }
...
class hat_trie<std::string> {
  ...
  public:
    ...
    bool insert(const key_type &key) {
        const std::string &word = refY<std::string>(key);
    ...
    iterator insert(const iterator &, const key_type &key) {
        const std::string &word = refX(key);
        insert(word);
        return find(word);
    }
