...
static value_t* get_key(ahtable_t* T, const char* key, size_t len, bool insert_missing, bool* inserted)
{
...
value_t* ahtable_get(ahtable_t* T, const char* key, size_t len, bool* inserted)
{
    return get_key(T, key, len, true, inserted);
}


value_t* ahtable_tryget(ahtable_t* T, const char* key, size_t len )
{
    return get_key(T, key, len, false, NULL);
}
