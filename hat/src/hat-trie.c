    ...
    while (!ahtable_iter_finished(i)) {
        ...
        if ((unsigned char) key[0] <= j) {
            if (*left.flag & NODE_TYPE_PURE_BUCKET) {
                v = ahtable_get(left.b, key + 1, len - 1, NULL);
            }
            else {
                v = ahtable_get(left.b, key, len, NULL);
            }
            *v = *u;
        }
        ...
        /* right */
            ...
            if (*right.flag & NODE_TYPE_PURE_BUCKET) {
                v = ahtable_get(right.b, key + 1, len - 1, NULL);
            }
            else {
                v = ahtable_get(right.b, key, len, NULL);
            }
            *v = *u;
        }
...
value_t* hattrie_get(hattrie_t* T, const char* key, size_t len, bool* inserted)
{
    node_ptr parent = T->root;
    assert(*parent.flag & NODE_TYPE_TRIE);
    ...
    value_t* val;
    if (*node.flag & NODE_TYPE_PURE_BUCKET) {
        val = ahtable_get(node.b, key + 1, len - 1, inserted);
    }
    else {
        val = ahtable_get(node.b, key, len, inserted);
    }
    T->m += (node.b->m - m_old);
