...
class AdaptiveRadixTree {
  ...
  protected:
    ...
    virtual void loadKey(uintptr_t tid, uint8_t key[]) = 0;
    Node* lookupValue(Node* node, uint8_t key[], unsigned keyLength, unsigned depth, unsigned maxKeyLength);
    Node* lookupValuePessimistic(Node* node, uint8_t key[], unsigned keyLength, unsigned depth, unsigned maxKeyLength);
